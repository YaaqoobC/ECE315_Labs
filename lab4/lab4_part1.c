...
#define PMOD_MOTOR_DEVICE_ID                XPAR_XGPIO_4_BASEADDR
...
static void _Task_Motor( void *pvParameters ){
	...
	decision_parameters *read_motor_parameters_from_queue_ptr;
	...
	while(1){
        ...
        xQueueReceive(xQueue_FIFO1, &read_motor_parameters_from_queue_ptr, 0);
        ...
        Stepper_setSpeedInStepsPerSecond(read_motor_parameters_from_queue_ptr->rotational_speed);
        Stepper_setAccelerationInStepsPerSecondPerSecond(read_motor_parameters_from_queue_ptr->rotational_acceleration);
        Stepper_setDecelerationInStepsPerSecondPerSecond(read_motor_parameters_from_queue_ptr->rotational_deceleration);
        Stepper_setCurrentPositionInSteps(read_motor_parameters_from_queue_ptr->currentposition_in_steps);
        
        for (int i = 0; i < sequenceIndex; ++i) {
            XGpio_DiscreteWrite(&Red_RGBInst, 2, LED_GREEN);
            int pos = positionSequence[i][0];

            int steps_to_move = read_motor_parameters_from_queue_ptr->currentposition_in_steps - pos;
            Stepper_moveRelativeInSteps(steps_to_move);
