        ...
        if (report_flag) {
            ...
            while(xQueueReceive(spi_to_uart, &spi_byte, 0)) {
                if (spi_byte == CHAR_DOLLAR) {
                    ...
                    // terminateInput() clears report_flag, resets loopback, and prints the exit message.
                    terminateInput(); 
                    break;
                } else {
                    // Print the actual report characters to your console!
...
static void vSpiSubTask(void *pvParameters)
{
    ...
    while (1) {
        if (spi_loopback && command_flag == 2) {
			// TODO 10: prepare for transmission
            ...
            for (i = 0; i < TRANSFER_SIZE_IN_BYTES; i++) {
                ...
                message_byte_count++;
                total_bytes_received_over_spi += 1;
				...
				if (terminationSequence(rolling)) {
                    ...
                    report_len = snprintf(report,
                                          sizeof(report),
                                          "\nNumber of bytes received over SPI:%d\nLast message byte count: %d\nTotal messages received: %d\n", 
                                            total_bytes_received_over_spi - (3*total_messages_received), last_message_byte_count, total_messages_received);
